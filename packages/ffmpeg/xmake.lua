...
if is_plat("windows", "mingw") then
    ...
    add_configs("shared",
        { description = "Download shared binaries.", default = true, type = "boolean", readonly = true })
    ...
    add_configs("vulkan", { description = "Enable vulkan.", default = false, type = "boolean" })
end
    ...
    on_fetch("mingw", "linux", "macosx", function(package, opt)
        ...
        if opt.system then
            ...
            local ffmpeg = find_tool("ffmpeg",
                { check = "-help", version = true, command = "-version", parse = "%d+%.?%d+%.?%d+", force = true })
            if ffmpeg then
...
on_install("linux", "macosx", "android@linux,macosx", function(package)
    ...
    if package:is_plat("android") then
        ...
        local cflags   = table.join(table.wrap(package:config("cxflags")), table.wrap(package:config("cflags")),
            table.wrap(get_config("cxflags")), get_config("cflags"))
        local cxxflags = table.join(table.wrap(package:config("cxflags")), table.wrap(package:config("cxxflags")),
            table.wrap(get_config("cxflags")), get_config("cxxflags"))
